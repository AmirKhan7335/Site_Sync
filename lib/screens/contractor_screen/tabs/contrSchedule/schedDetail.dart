...
import '../../../consultant_screens/activityGallery.dart';
...
class ContrScheduleDetail extends StatefulWidget {
  ContrScheduleDetail({required this.projId, required this.title, required this.projType ,super.key});
  String projId;
  String title;
  String projType;
  @override
...
class _ConsultantScheduleState extends State<ContrScheduleDetail> {
  ...
  Widget _buildActivityContainer(
      Activity activity, String mainHeading, String subHeading, String id) {
    ...
    return InkWell(
      onTap: () {
        if(widget.projType == 'contractor'){
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => ActivityGallery(
                activityName: capitalize(activity.name),
                engEmail: engEmail,  // Pass the engEmail from state
                activityId: activity.id, // Pass the activityId from the activity object
              ),
            ),
          );
        }
          ...
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => DetailsScreen(
                  mainHeading: capitalize(activity.name),
                  subHeading: subHeading,
                  image: activity.image, activityId: activity.id,engineerEmail: engEmail ),
            ),
          );
        }
      },
