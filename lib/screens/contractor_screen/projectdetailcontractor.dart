...
class ProjectDetailContractor extends StatefulWidget {
  ProjectDetailContractor({required this.projectDataList, required this.engineerName,super.key,});

  List projectDataList;
  String engineerName;
  @override
  State<ProjectDetailContractor> createState() => _RequestBodyState();
}

class _RequestBodyState extends State<ProjectDetailContractor> {
  ...
  Future<void> fetchClientandContractorName() async {
    try {
      ...
      // Fetch projects collection
      final projectsSnapshot1 = await FirebaseFirestore.instance.collection('users').where('name', isEqualTo: contractorName1).get();

      // Check if project exists
      if (projectsSnapshot1.docs.isEmpty) {
        throw Exception('Project not found');
      }


      // Assume only one project matches the project name
      final projectDoc1 = projectsSnapshot1.docs.first;
      final projectData1 = projectDoc1.data();

      // Retrieve the contractor name from the project document, or set it to empty string if not found
      final contractorName = projectData1.containsKey('companyName') ? projectData1['companyName'] : 'No contractor Enrolled yet';
